...
export async function decimate(geometry, targetTriangles, maxError, onProgress, selection) {
  const { positions, faces, vertCount, faceCount, numUnique, uniqueIDs } = buildIndexed(geometry);

  if (faceCount <= targetTriangles) {
    const geo = buildOutput(positions, faces, faceCount);
    if (selection) geo.setAttribute('fs_selection', new THREE.BufferAttribute(new Float32Array(selection), 1));
    return geo;
  }
  ...
  const vertInSelected = applySelectionFreeze(quadrics, selection, vertCount, faceCount, uniqueIDs);

  // Build a per-FACE selection array from the original input selection.
  // This is the ground truth and must NOT be derived from vertex data after collapses.
  let faceSelected = null;
  if (selection) {
    faceSelected = new Uint8Array(faceCount);
    for (let f = 0; f < faceCount; f++) {
      // selection is per-vertex (non-indexed), 3 values per face, all set to the same value.
      faceSelected[f] = selection[f * 3] > 0.5 ? 1 : 0;
    }
  }
  ...
  return buildOutput(positions, faces, faceCount, faceSelected);
}
...
// ── Selection Freeze ─────────────────────────────────────────────────────────
// Freezes unselected geometry by applying a massive penalty to their quadrics.
// Also adds a moderate penalty to boundary vertices (shared between selected
// and unselected faces) to prevent QEM from collapsing edges near the selection
// boundary, which causes star-shaped distortion around holes and cutouts.
function applySelectionFreeze(quadrics, selection, vertCount, faceCount, uniqueIDs) {
  if (!selection || selection.length === 0) return null;

  const vertInSelected   = new Uint8Array(vertCount);
  const vertInUnselected = new Uint8Array(vertCount);
  let anyUnselected = false;

  for (let f = 0; f < faceCount; f++) {
    if (selection[f * 3] > 0.5) {
      for (let j = 0; j < 3; j++) {
        vertInSelected[uniqueIDs[f * 3 + j]] = 1;
      }
    } else {
      anyUnselected = true;
      for (let j = 0; j < 3; j++) {
        vertInUnselected[uniqueIDs[f * 3 + j]] = 1;
      }
    }
  }

  if (anyUnselected) {
    const FREEZE           = 1e8;   // Total freeze for purely unselected vertices
    const BOUNDARY_PENALTY = 1e4;   // Moderate penalty for selection-boundary vertices
    for (let u = 0; u < vertCount; u++) {
      if (vertInSelected[u] === 0) {
        // Purely unselected vertex — complete freeze
        quadrics[u * 10]     += FREEZE;
        quadrics[u * 10 + 4] += FREEZE;
        quadrics[u * 10 + 7] += FREEZE;
      } else if (vertInUnselected[u] === 1) {
        // Boundary vertex — belongs to both selected AND unselected faces.
        // Moderate penalty discourages QEM from collapsing edges here,
        // preventing the star-shaped convergence around holes/cutouts.
        quadrics[u * 10]     += BOUNDARY_PENALTY;
        quadrics[u * 10 + 4] += BOUNDARY_PENALTY;
        quadrics[u * 10 + 7] += BOUNDARY_PENALTY;
      }
    }
  }
  return vertInSelected;
}
...
function buildIndexed(geometry) {
  ...
  return { positions: positions.subarray(0, vertCount * 3), faces, vertCount, faceCount, numUnique: vertCount, uniqueIDs: indexRemap };
}

// (adjacency helpers replaced by buildLinkedAdj and _unlinkSlot/_moveSlot above)

function buildOutput(positions, faces, faceCount, faceSelected = null) {
  let activeFaces = 0;
  for (let f = 0; f < faceCount; f++) {
  ...
  let selArray = null;
  if (faceSelected) {
    selArray = new Float32Array(activeFaces * 3);
  }
  ...
  for (let f = 0; f < faceCount; f++) {
    ...
    if (selArray) {
      // Use the per-face selection stamp directly — this was set from the original input
      // and is immune to vertex identity changes caused by QEM edge collapses.
      const isSel = faceSelected[f] ? 1.0 : 0.0;
      selArray[sOut++] = isSel;
      selArray[sOut++] = isSel;
      selArray[sOut++] = isSel;
    }
  ...
  if (selArray) {
    geo.setAttribute('fs_selection', new THREE.BufferAttribute(selArray, 1));
  }
  return geo;
}
