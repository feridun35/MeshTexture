...
const vertexShader = /* glsl */`
  ...
  // We gracefully handle absence of custom attributes via a trick or just defining them 
  // and defaulting manually if not present, but three.js usually supplies 0.0 vectors.
  attribute vec3  smoothNormal;
  attribute vec3  faceNormal;
  attribute float faceMask;
  // Fallback for Folder A selection mask:
  attribute float fs_selection;
  ...
  void main() {
    ...
    // Apply Planar Align Projection Matrix to World position
    vec4 worldPos = modelMatrix * vec4(position, 1.0);
    vec3 alignedPos = (planarProjMat * worldPos).xyz;
    vec3 pos = position;

    // Apply Planar projection matrix to Normal
    vec3 wNorm = normalize(mat3(modelMatrix) * safeN);
    vec3 alignedNrm = normalize(mat3(planarProjMat) * wNorm);
    ...
    // Handle both FaceMask (Folder B) and fs_selection (Folder A) naturally
    float userMask = 1.0;
    if (faceMask == 0.0 && fs_selection == 0.0) {
        userMask = 1.0; // No mask attributes found, default to visible
    } else if (fs_selection > 0.0) {
        userMask = fs_selection; // Folder A uses 1.0 for selected
    } else if (faceMask > 0.0) {
        userMask = faceMask; // Folder B might use FaceMask, but we rely on fs_selection
    }
    // Since Folder A uses fs_selection = 0 for unselected, 1 for selected:
    // If fs_selection is present, then userMask becomes fs_selection.

    float totalMask = angleMask * fs_selection; // Use fs_selection strictly for Folder A
    vFaceMask = totalMask;

    if (useDisplacement == 1) {
      float h = computeHeightAtPoint(alignedPos, alignedNrm, alignedNrm);
    ...
    // Pass ALIGNED position for UV computation in the fragment shader.
    vModelPos    = alignedPos;
    vModelNormal = alignedNrm;
...
const fragmentShader = /* glsl */`
  ...
  void main() {
    ...
    // F-HardCut: We enforce a strict binary cutoff in the fragment shader to eliminate ANY 
    // soft interpolation artifacts exactly like in the CPU worker.
    float maskBlend = vFaceMask >= 0.5 ? 1.0 : 0.0;
    ...
    // ── Shading ───────────────────────────────────────────────────────────
    vec3 baseColor = mix(vec3(0.50, 0.50, 0.50), vec3(0.00, 0.533, 1.00), maskBlend);
...
export function updateMaterial(material, displacementTexture, settings) {
  ...
  if (!u) return;
  if (displacementTexture && u.displacementMap && u.displacementMap.value !== displacementTexture) {
    u.displacementMap.value = displacementTexture;
  }
  if(u.mappingMode) u.mappingMode.value   = settings.mappingMode;
  if(u.scaleUV) u.scaleUV.value.set(settings.scaleU, settings.scaleV);
  if(u.amplitude) u.amplitude.value     = settings.amplitude;
  if(u.offsetUV) u.offsetUV.value.set(settings.offsetU, settings.offsetV);
  if(u.rotation) u.rotation.value      = (settings.rotation ?? 0) * Math.PI / 180;
  if (settings.bounds && u.boundsMin) {
  ...
  if(u.bottomAngleLimit) u.bottomAngleLimit.value = settings.bottomAngleLimit ?? 5.0;
  if(u.topAngleLimit) u.topAngleLimit.value    = settings.topAngleLimit    ?? 0.0;
  if(u.mappingBlend) u.mappingBlend.value            = settings.mappingBlend            ?? 0.0;
